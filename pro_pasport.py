...
def recognize_passport(img: str, user_id: str) -> dict:
    ...
    info = [f"file/{user_id}/crops/first/" + os.listdir(f"file/{user_id}/crops/first/")[0],
    ...
    data = {"Имя": "".join(reader.readtext(info[0], detail=0)).lower().capitalize(),
            ...
            "Номер": "".join(reader.readtext(info[3], detail=0)).lower(),
            "Серия": "".join(reader.readtext(info[4], detail=0)).lower(), }
